...
class data_analy():
    ...
    def filter_data(file,filter_len,threshold, date, show=True,savePdf=False, transport='bike'):
        """
        ...
        if show==True:
            ...
            #raw data
            ax1.plot(time,data, '.', ms=2,color='grey')
            ...
            ax3.set_ylabel("Standard deviation \n of background (ppm)", fontsize=10)
            ax3.tick_params(axis='y', which='major', labelsize=15)
            ax3.legend()  
            
            #newbackground
            ax4.plot(time,data,'.',ms=2,color='grey')
            ax4.plot(time,newBg, '.', ms=2)
            ax4.set_ylabel("Adjusted Background (ppm)", fontsize=10)
            ax4.set_ylim((1.8,3))
    ...
    def show_peaks(file, filter_len, threshold, date, mode=1, show=True,savePdf=False,lim=[0.04,0.2,1]):
        """
        (file, int, int, str, bool) -> 
    ...
    def areaPeaks(file, filter_len, threshold, date, mode=1, show=True,limA=[1,4,60], limH=[0.04,0.2,1],transport='bike'):
        ...
        time_data, data, background, anomaly, _=data_analy.filter_data(file,filter_len,threshold,
                                                                      date, show=show,savePdf=False, transport=transport)
        ...
        for block in blocks.T:
            ...
            area=sum(anomaly[block[0]:block[1]])
            ...
            list_data = [ti, peakM, lat, lon, r, wd_corr, ws_corr]
            list_peaks.append(list_data)
    ...
    def analyze_data(file, date, mode=1,savePeak=True,lim=[0.04,0.4,1],transport='bike'):
        ...
        time, data, background, anomaly,_ = data_analy.filter_data(file, filter_len, threshold, date, show=True, savePdf=False,transport=transport)
        
        time, data, background, anomaly, blocks = data_analy.show_peaks(file, filter_len, threshold, date, mode=mode, show=True, savePdf=False,lim=lim)
    ...
    def clean_file(file):
        ...
        for line in f:
            var = line.split(',')
            data.append([var[0], float(var[1]), float(var[2]), float(var[3]), int(var[4]), float(var[5]), float(var[6])])
            #this is a list of [date, enh, lat, lon, level, wd_corr, ws_corr]
        ...
        with open(file_name, 'w') as csvfile:
            ...
            for peak in data:
                ...
                i += 1
                for peak2 in data[i:]:
                    dis = distance_between(peak[2], peak[3], peak2[2], peak2[3])
                    t_diff = peak2[5] - peak[5]
